...
//Принимает указатель на родительский виджет в качестве параметра, а также задает изгачальные настройки данному виджету.
...
// Метод-обработчик события нажатия на кнопку мыши. Снимает подсветку со всех узлов дерева.
...
// Метод-обработчик события нажатия на кнопку "Сформировать". Формирует новое идеально-сбалансированное дерево поиска,
...
// Метод-обработчик события нажатия на кнопку "Удалить". Ищет в дереве элемент с заданным значением и,
// если тот найден, то удаляет его из дерева. Если элемент не найден, то выводится соотвествующее сообщение.
...
// Метод-обработчик события нажатия на кнопку "Преобразовать в дерево поиска". Вызывает метод конвертации дерева в дерево поиска.
...
// Метод для вывода сообщения в данном виджете. Выводит переданный текст на специальной панели сообщений.
